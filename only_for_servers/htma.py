...
"""
In linux-like servers, htma.py must be into the /var/www/ folder, and the target path aiming to "html"
"""
                ...
                if 'id="' in htm[i] or "id='" in htm[i]:
                    ...
                    if 'src="' in htm[i] or "src='" in (htm[i]):
                        ...
                        if "://" in matches[0].replace("'", "").replace('"', ""):
                            ...
                            output[htma_id.replace("'", "").replace('"', "")] = ip+webname + os.sep +  os.path.basename(self.attributes["DIR"])  + os.sep + matches[0].replace("'", "").replace('"', "")
                    else:
    ...
    def get_format_global (self, output, path=""):
        ...
        if path:
            ...
            if self.attributes["REDIR"] == "MD":
                output = output.replace ("$LINK_FOR_EACH_FILE_IN_DIR$",ip+webname+ path[path.find(os.sep):].replace(".md",".html"))
                print (path[path.find(os.sep):])
            elif  self.attributes["REDIR"] == "HTMA":
                 output = output.replace ("$LINK_FOR_EACH_FILE_IN_DIR$",ip+webname+path[path.find(os.sep):].replace(".htma",".html"))
            else:
...
class HTMAProject ():
    ...
    def generate_target (self):
        ...
        try:
            shutil.rmtree (self.target+webname)
        except: 
            print ("Warning: Directory webname does no exist in specified directory, creating...")

        # Generación de los ficheros vacíos, para evitar errores posteriores asociados a format_md

        for root, dirs, files in os.walk(self.path):
            target_directory = root.replace(self.path, self.target+webname)
