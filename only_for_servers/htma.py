                ...
                if 'id="' in htm[i] or "id='" in htm[i]:
                    ...
                    if 'src="' in htm[i] or "src='" in (htm[i]):
                        ...
                        if "://" in matches[0].replace("'", "").replace('"', ""):
                            ...
                            output[htma_id.replace("'", "").replace('"', "")] = ip+os.path.basename(target_path)+os.sep+"target" + os.sep +  os.path.basename(self.attributes["DIR"])  + os.sep + matches[0].replace("'", "").replace('"', "")
                    else:
        ...
        def md_ids (path):  
            ...
            for line in md:
                ...
                # MD_IMG
                    ...
                    output ["MD_IMG"+"["+str(img_counter)+"]"] = ip+os.path.basename(target_path)+os.sep+"target" + os.sep + os.path.basename(self.attributes["DIR"])  + os.sep +  find [0].replace ("(", "").replace(")","")

                # MD_URL
                elif "[" in line and  "](" in line and not r"\[" in line:
                    url_counter = img_counter +1
                    find = re.findall(r'\(.*?\)', line)
                    output ["MD_URL"+"["+str(url_counter)+"]"] =  ip+os.path.basename(target_path)+os.sep+"target" + os.sep + os.path.basename(self.attributes["DIR"])  + os.sep +  find [0].replace ("(", "").replace(")","").replace(".md", ".html") # Componenda
    ...
    def get_format_global (self, output, path=""):
        ...
        if path:
            ...
            if self.attributes["REDIR"] == "MD":
                output = output.replace ("$LINK_FOR_EACH_FILE_IN_DIR$",ip+os.path.basename(target_path)+os.sep+"target"+ path[path.find(os.sep):].replace(".md",".html"))
                print (path[path.find(os.sep):])
            elif  self.attributes["REDIR"] == "HTMA":
                 output = output.replace ("$LINK_FOR_EACH_FILE_IN_DIR$",ip+os.path.basename(target_path)+os.sep+"target"+path[path.find(os.sep):].replace(".htma",".html"))
            else:
                output = output.replace ("$LINK_FOR_EACH_FILE_IN_DIR$",ip+os.path.basename(target_path)+os.sep+"target"+path[path.find(os.sep):])
